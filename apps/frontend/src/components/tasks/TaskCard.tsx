...
export function TaskCard({ task, onClick, isDragging }: TaskCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`bg-card border border-border rounded p-3 md:p-4 cursor-pointer hover:border-border/60 transition-smooth shadow-sm ${
        isDragging ? "opacity-50 rotate-2 shadow-lg" : ""
      }`}
    >
      <div className="flex items-start justify-between gap-2 mb-1.5 md:mb-2">
        <h3 className="text-card-foreground flex-1 line-clamp-2 text-sm md:text-base">
          {task.title}
        </h3>
      ...
      {task.description && (
        <p className="text-muted-foreground text-xs mb-2 md:mb-3 line-clamp-2">
          {task.description}
        </p>
      )}
      ...
      <div className="flex items-center justify-between pt-2 md:pt-3 border-t border-border gap-2">
        ...
        {dueDate && (
          <div
            className={`flex items-center gap-1 text-xs flex-shrink-0 ${isOverdue ? "text-destructive" : "text-muted-foreground"}`}
          >
