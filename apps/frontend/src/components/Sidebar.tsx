...
export function Sidebar({ isMobileOpen = false, onMobileClose }: SidebarProps) {
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.href}
              className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded transition-smooth relative group ${
                isActive
  ...
  return (
    <ErrorBoundary>
      <>
        {/* Desktop sidebar */}
        <div className="hidden lg:block sticky top-0">{sidebarContent}</div>
