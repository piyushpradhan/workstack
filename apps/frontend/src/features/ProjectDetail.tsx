...
const ProjectDetail = () => {
  ...
  const members = allProjectUsers.filter((u) =>
  ...
  const tasksByStatus = {
    ...
    inProgress: projectTasks.filter((t: Task) => t.status === "IN_PROGRESS")
      .length,
    inReview: projectTasks.filter((t: Task) => t.status === "IN_REVIEW").length,
    done: projectTasks.filter((t: Task) => t.status === "DONE").length,
    cancelled: projectTasks.filter((t: Task) => t.status === "CANCELLED")
      .length,
  };
