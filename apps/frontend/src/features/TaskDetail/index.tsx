...
const TaskDetail = () => {
    ...
    const project = useMemo(() => projects.find(p => p.id === task?.projectId) ?? null, [projects, task]);
    const owner = useMemo(() => allProjectUsers.find(u => u.id === task?.ownerId) ?? null, [allProjectUsers, task]);
    const memberUsers = useMemo(() => allProjectUsers.filter(u => (task?.memberIds ?? []).includes(u.id)), [allProjectUsers, task]);
    ...
    const handleDueDateChange = (dueDate: string) => {
        if (!task) return;
        updateTaskMutation.mutate({ id: task.id, data: { dueDate: dueDate ? new Date(dueDate).toISOString() : undefined } });
    ...
    const handleDescriptionChange = (nextDescription: string) => {
        if (!task) return;
        updateTaskMutation.mutate({ id: task.id, data: { description: nextDescription } });
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <TaskComments
                            ...
                            onAdd={(content) => { setCommentContent(content); handleAddComment(); }}
                            onAddDirect={(content) => {
                                if (!content.trim()) { toast.error('Comment cannot be empty'); return; }
                                const newComment = { id: `temp-${Date.now()}`, userId: currentUser?.id ?? 'unknown', content: content.trim(), createdAt: new Date().toISOString() };
                                setComments((prev) => [newComment, ...prev]);
                            }}
