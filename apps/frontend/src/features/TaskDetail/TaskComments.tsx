...
export function TaskComments({ comments, currentUserName, onAdd, onAddDirect, onDelete, formatDateTime, users }: TaskCommentsProps) {
    ...
    return (
        <div className="bg-card border border-border rounded-lg p-5 sm:p-6 shadow-sm">
            ...
            <div className="mb-6">
                <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex justify-end">
                            <Button
                                onClick={(e) => {
                                    const el = (e.currentTarget.closest('div')?.previousSibling as HTMLTextAreaElement) ?? null;
                ...
                <p className="text-muted-foreground mt-2 ml-11">Press ⌘/Ctrl + Enter to submit</p>
            </div>
                    ...
                    comments.map((comment) => {
                        ...
                        return (
                            <div key={comment.id} className="flex gap-3 group">
                                <Avatar className="w-8 h-8 bg-primary flex items-center justify-center shrink-0">
                                    <span className="text-primary-foreground">{(user?.name ?? '?').charAt(0).toUpperCase()}</span>
                                </Avatar>
                                <div className="flex-1 min-w-0">
                                    <div className="bg-background border border-border rounded-lg p-4 shadow-sm">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                <span className="text-foreground">{user?.name ?? 'Unknown'}</span>
                                                {isOwner && (<span className="text-muted-foreground px-2 py-0.5 bg-accent rounded text-xs">You</span>)}
                                            </div>
                                            <div className="flex items-center gap-2">
