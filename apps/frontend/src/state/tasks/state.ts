...
export class TaskStateManager {
  ...
  getTasksByProjectId(projectId: string): Task[] {
    return (
      queryClient.getQueryData<Task[]>(stateKeys.tasks.byProject(projectId)) ??
      []
    );
  }
  ...
  getTasksByUserId(userId: string): Task[] {
    return (
      queryClient.getQueryData<Task[]>(stateKeys.tasks.byUser(userId)) ?? []
    );
  }

  getTasksByStatus(status: string): Task[] {
    return (
      queryClient.getQueryData<Task[]>(stateKeys.tasks.byStatus(status)) ?? []
    );
  }

  getTasksByPriority(priority: string): Task[] {
    return (
      queryClient.getQueryData<Task[]>(stateKeys.tasks.byPriority(priority)) ??
      []
    );
  }
  ...
  getTasksBySearch(search: string): Task[] {
    return (
      queryClient.getQueryData<Task[]>(stateKeys.tasks.search(search)) ?? []
    );
  }
}
