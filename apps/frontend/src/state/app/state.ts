...
export class AppStateManager {
  ...
  getSettings(): AppSettings {
    return (
      queryClient.getQueryData<AppSettings>(stateKeys.app.settings()) ?? {
        theme: "system",
        language: "en",
        profileDisplayName: "",
  ...
  getNotifications(): AppNotification[] {
    return (
      queryClient.getQueryData<AppNotification[]>(
  ...
  getOnlineStatus(): boolean {
    return (
      queryClient.getQueryData<boolean>(["app", "isOnline"]) ?? navigator.onLine
    );
  }
  ...
  getLastActivity(): number {
    return (
      queryClient.getQueryData<number>(["app", "lastActivity"]) ?? Date.now()
    );
  }
