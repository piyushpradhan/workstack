...
const cookieOptions = ({
  ...
  const age = lifespan === 'access' ? config.JWT_EXPIRY : config.JWT_REFRESH_EXPIRY;

  return {
    path,
    secure: config.APP_ENV !== 'development',
    sameSite: 'lax',
    domain: config.APP_ENV === 'development' ? 'localhost' : (config.APP_URL ? `.${config.APP_URL}` : undefined),
...
export const isFromFrontend = (request: FastifyRequest) => request.headers['x-requested-with'] === 'XMLHttpRequest';
...
export const sendAccessTokenAndSessionId = (reply: FastifyReply, { accessToken, sessionId, redirectTo }: SendAccessTokenAndSessionId) => {
...
export default fp(async (fastify) => {
  ...
  fastify.decorateReply('sendAccessTokenAndSessionId', sendAccessTokenAndSessionId)
})
