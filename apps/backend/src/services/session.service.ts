...
export class SessionService {
  ...
  refreshSession = async ({ sessionId, nonce, nextNonce }: UpdateSession) => {
    ...
    try {
      ...
      if (error instanceof PrismaClientKnownRequestError) {
        if (error.code === 'P2023') {
          throw new httpErrors.Unauthorized('You have been logged out. Please login again')
        }
      }
    ...
    if (session.expiresAt < now) {
      throw new httpErrors.Unauthorized('Session has expired. Please login again')
    }
  ...
  listSessions = async ({ userId }: { userId: string }) => this.session.findMany({
    where: { userId },
  })
